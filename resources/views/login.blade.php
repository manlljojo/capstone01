...
<div class="row justify-content-center mt-5 mb-5 animate__animated animate__fadeIn">
    <div class="col-lg-5">
        <div class="card p-5 shadow border-0" style="border-radius: 1.5rem; background: rgba(255,255,255,0.95); backdrop-filter: blur(10px);">
            ...
            @if(session('error'))
                <div class="alert alert-danger border-0 shadow-sm rounded-4 animate__animated animate__shakeX mt-3">
            ...
            <form action="{{ route('login') }}" method="post">
                @csrf
                <div class="mb-3">
                    <label class="form-label text-muted fw-bold small">ID / USERNAME</label>
                    <input class="form-control rounded-pill px-4" style="border: 2px solid #eee; height: 50px;" type="text" placeholder="Masukkan ID Anda..." name="username" required>
                </div>
                <div class="mb-4">
