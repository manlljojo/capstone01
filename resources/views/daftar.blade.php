                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0 text-center">
                        ...
                        <tbody>
                            ...
                            <tr class="border-bottom {{ $data->status_pemesanan === 'Check-in' ? 'bg-light' : '' }}">
                                <td class="text-start ps-4">
                                    <span class="fw-bold d-block">{{ $data->nama_pemesan ?? ($data->pengguna->nama ?? 'Tanpa Nama') }}</span>
                                    @php
                                        $firstEvent = $data->detailPemesanans->first()->tiket->event ?? null;
                                    @endphp
                                    <small class="text-muted"><i class="fa fa-ticket-alt text-warning me-1"></i>{{ $firstEvent->nama_event ?? '-' }} (#{{ $data->id_pemesanan }})</small>
                                </td>
                                <td>
                                    <span class="fw-bold">Rp {{ number_format($data->total_harga, 0, ',', '.') }}</span> <br>
                                    <span class="badge bg-light text-muted border small">{{ $data->pembayaran->metode_pembayaran ?? '-' }}</span>
                                </td>
                                <td>
                                    @if(($data->pembayaran->status_bayar ?? '') !== 'Lunas')
                                        <form action="/admin/confirm-payment/{{ $data->id_pemesanan }}" method="POST" class="d-inline">
                                            @csrf
                                ...
                                <td>
                                    <form action="/admin/checkin/{{ $data->id_pemesanan }}" method="POST" class="m-0">
                                        @csrf
                                        @if(($data->pembayaran->status_bayar ?? '') == 'Lunas')
                                            @if($data->status_pemesanan != 'Check-in')
